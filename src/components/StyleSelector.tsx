  ...
  return (
    <div className='relative space-y-3 dark'>
        ...
        <button type='button' onClick={()=>setisOpen(!isOpen)}
            className='flex w-full items-center justify-between rounded-md border px-4 py-3 text-left transition bg-white/8 border-white/8 text-zinc-200  hover:bg-white/12'>
            ...
            isOpen && (
                <div className='absolute bottom-0 z-50 mt-1 w-full rounded-md border border-white/12 bg-black/20  backdrop-blur-3xl shadow-lg'>
                    {thumbnailStyles.map((style)=>(
                        <button 
                        className='fles w-full item-start gap-3 px-4 py-4 text-left transition hover:bg-black/30'
